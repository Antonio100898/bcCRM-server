<%@ Page Title="" Language="C#" MasterPageFile="~/MainMaster.master" AutoEventWireup="true" CodeFile="ShowProblems.aspx.cs" Inherits="ShowProblems" MaintainScrollPositionOnPostback="true" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <div style="text-align: center; width: 100%" class="alert-info">
        <asp:Label ID="lblHeader" runat="server" Text="" Font-Size="X-Large" Font-Bold="true" />        
    </div>
    <div id="divProblems" runat="server" class="alert-info">
        <asp:GridView ID="grdProblems" runat="server" CssClass="table table-bordered table-hover" AutoGenerateColumns="False" DataKeyNames="id" Width="100%" AllowSorting="True" DataSourceID="dsOpenProblems"
            OnRowUpdating="grdProblems_RowUpdating" OnRowUpdated="grdProblems_RowUpdated" OnRowDataBound="grdProblems_RowDataBound">
            <Columns>
                ...
                <asp:TemplateField HeaderStyle-CssClass="hidden-xs" ItemStyle-CssClass="hidden-xs">
                    <ItemTemplate>
                        <asp:Button ID="btn_Edit" runat="server" CssClass="btn-primary" Text="ערוך" CommandName="Edit" Font-Size="Large" />
                        <div style="text-align: center">
                            <asp:ImageButton ID="btnShowLog" runat="server" Visible='<%# Eval("HaveLog") %>' CommandName="ShowLog" CommandArgument='<%# Eval("id") %>' ImageUrl="~/Pics/log.png" Width="25px" Height="25px" AlternateText='<%# Eval("id") %>' OnClick="btnShowLog_Click" />
                        </div>
                    </ItemTemplate>
                    <EditItemTemplate>
                        <asp:Button ID="btn_Update" runat="server" Width="100%" Text="עדכן" CommandName="Update" CssClass="btn-success" Font-Size="Large" />
                        <asp:Button ID="btn_Cancel" runat="server" Width="100%" Text="בטל" CommandName="Cancel" CssClass="btn-danger" Font-Size="Large" />
                    </EditItemTemplate>
                </asp:TemplateField>
            </Columns>
            <HeaderStyle CssClass="text-center" />
        </asp:GridView>
        <asp:SqlDataSource runat="server" ID="dsOpenProblems" ConnectionString='<%$ ConnectionStrings:BeecommDBConnectionString %>'
            SelectCommand="SELECT problemsClose.id, problemsClose.workerId, problemsClose.phoneId, problemsClose.phone, problemsClose.ip, problemsClose.placeNameId, problemsClose.placeName, problemsClose.customerName, problemsClose.problemDesc, problemsClose.problemSolution, problemsClose.statusId, problemsClose.emergencyId, problemsClose.departmentId, problemsClose.reportToYaron, problemsClose.startTime, problemsClose.finishTime, problemStatus.statusName, problemsClose.HaveLog, workers.firstName + N' ' + workers.lastName AS workerName, departments.departmentName, emergencyTypes.emergencyName, problemsClose.toWorker, workers_1.firstName + N' ' + workers_1.lastName AS toWorkerName FROM problemsClose INNER JOIN workers ON problemsClose.workerId = workers.id INNER JOIN problemStatus ON problemsClose.statusId = problemStatus.id INNER JOIN emergencyTypes ON problemsClose.emergencyId = emergencyTypes.id INNER JOIN departments ON problemsClose.departmentId = departments.id INNER JOIN workers AS workers_1 ON problemsClose.toWorker = workers_1.id WHERE (problemsClose.statusId = 0 OR problemsClose.statusId = 1) ORDER BY problemsClose.startTime DESC"
            UpdateCommand="UPDATE problemsClose SET ip = @ip, placeNameId = @placeNameId, placeName = @placeName, customerName = @customerName, problemDesc = @problemDesc, problemSolution = @problemSolution, toWorker = @toWorker, statusId = @statusId, emergencyId = @emergencyId, departmentId = @departmentId, reportToYaron = @reportToYaron, finishTime = GETDATE() WHERE (id = @id)">
            <UpdateParameters>
                <asp:Parameter Name="ip" />
                <asp:Parameter Name="placeNameId" />
                <asp:Parameter Name="placeName" />
                <asp:Parameter Name="customerName" />
                <asp:Parameter Name="problemDesc" />
                <asp:Parameter Name="problemSolution" />
                <asp:Parameter Name="toWorker" />
                <asp:Parameter Name="statusId" />
                <asp:Parameter Name="emergencyId" />
                <asp:Parameter Name="departmentId" />
                <asp:Parameter Name="reportToYaron" />
    ...
    <div>
        <asp:UpdatePanel ID="UpdatePanel2" runat="server">
            <ContentTemplate>
                <%-- visible="false" class="panel-primary col-lg-12 Absolute-Center is-Responsive" style="position: fixed; top: 200px; padding: 20px; width: 100%"--%>
                <div id="divLog" runat="server" class="web_dialog">
                    <table style="width:100%">
                        <tr>
                            <td class="web_dialog_title">לוג</td>
                        </tr>
                    </table>
                    <div class="web_dialog_overlay"><strong>לוג</strong></div>                    
